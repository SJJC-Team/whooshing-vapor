...
public protocol HTTPIOHandler {
    /// 当接受到请求时，解析传来的请求。通常是进行解密操作，以将密文转为下一步可解析的 HTTP 报文
    func input(request: Data) throws -> Data
    /// 当该服务器对客户端做出响应时，包装将要做出的响应。通常是进行加密操作，以保护响应不被窃取
    func output(response: Data) throws -> Data
    /// 当接受到请求时，解析传来的请求。通常是进行解密操作，以将密文转为下一步可解析的 HTTP 报文。免去 ByteBuffer 与 Data 互转的步骤，直接操作 ByteBuffer，更加轻量
    func input(request: ByteBuffer) throws -> ByteBuffer
    /// 当该服务器对客户端做出响应时，包装将要做出的响应。通常是进行加密操作，以保护响应不被窃取。免去 ByteBuffer 与 Data 互转的步骤，直接操作 ByteBuffer，更加轻量
    func output(response: ByteBuffer) throws -> ByteBuffer
}

public extension HTTPIOHandler {
    
    func input(request: Data) throws -> Data { request }
    
    func output(response: Data) throws -> Data { response }
    
    func input(request: ByteBuffer) throws -> ByteBuffer {
        let req = Data(buffer: request)
        return try dataToByteBuffer(data: input(request: req))
    }
    
    func output(response: ByteBuffer) throws -> ByteBuffer {
        let res = Data(buffer: response)
        return try dataToByteBuffer(data: output(response: res))
    }
...
final class CustomCryptoIOHandler: ChannelDuplexHandler {
    ...
    func channelRead(context: ChannelHandlerContext, data: NIOAny) {
        ...
        do {
            let req = try ioHandler.input(request: buffer)
    ...
    func write(context: ChannelHandlerContext, data: NIOAny, promise: EventLoopPromise<Void>?) {
        ...
        if i == 3 {
            ...
            i = 0
            do {
                let res = try ioHandler.output(response: res)
